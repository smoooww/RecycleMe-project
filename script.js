...
function generateItems(){
    ...
    let randomItem = itemBox[Math.floor(Math.random() * itemBox.length)]
    items.appendChild(randomItem);
    function fallDownItem() {
        if (itemBottom < basketBottom + 80 && itemBottom > basketBottom && itemLeft > basketLeft - 60 && itemLeft < basketLeft + 103) {
            items.removeChild(randomItem);
            clearInterval(fallInterval);
        if (randomItem == itemOne || randomItem == itemTwo || randomItem == itemFour || randomItem == itemFive || randomItem == itemSix || randomItem == itemSeven || randomItem == itemEight) {
            itemsCollected += 1;
        }
        else {
            itemsWrong += 1;
...
if (viewportWidth < 800) {
    ...
    btnLeft.onclick = function control() {
        moveBasketLeft();
    }

    var btnRight = document.getElementById('btnRight')
    btnRight.onclick = function control() {
        moveBasketRight();
    }
  
function generateItems(){
    var itemBottom = 415;
    var itemLeft = Math.floor(Math.random() * 300);
    ...
    items.appendChild(randomItem);
    function fallDownItem() {
        if (itemBottom < basketBottom + 50 && itemBottom > basketBottom && itemLeft > basketLeft - 40 && itemLeft < basketLeft + 65) {
            items.removeChild(randomItem);
            clearInterval(fallInterval);
        if (randomItem == itemOne || randomItem == itemTwo || randomItem == itemFour || randomItem == itemFive || randomItem == itemSix || randomItem == itemSeven || randomItem == itemEight) {
            itemsCollected += 1;
